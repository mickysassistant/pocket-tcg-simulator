...
### CLI (tcgp) - Feature Branch: feature/tcgp-cli-roadmap
- **CLI Entry Point:** `bin/tcgp` → `src/cli/index.js`
- **CLI Structure:**
  - `src/cli/index.js` - Main CLI entry point with command parser
  - `src/cli/commands/` - Individual command handlers
    - `version.js` - Show version information
    - `help.js` - Show help information
  - `tests/cli/` - CLI tests
- **CLI Pattern:**
  - Custom argument parser (not yargs - had issues with Node v22)
  - All commands support `--json` flag for JSON output
  - All commands support `--help` flag for command-specific help
  - Error messages include `error` and `reason` fields
- **CLI Execution:**
  - `node bin/tcgp <command> [options]`
  - Once npm installed: `tcgp <command> [options]`
...
### Testing Pattern
- **Test Runner:** Custom runner with manual assertions
- **Run Tests:** `node tests/cli/bootstrap.test.js` or `npm test`
- **Test Structure:**
  - Manual assertions using `assert.strictEqual` and `assert` functions
  - Test counter and summary output
  - Each test wrapped in `runTest()` helper
- **CLI Tests:** Use `execSync` to spawn CLI process and verify output
