...
export function canPlayToActive(state, playerId, handIndex) {
  ...
  if (handIndex < 0 || handIndex >= player.hand.length) return { valid: false, reasonKey: 'errors.validation.invalidHandIndex' };
...
export function canAttachEnergy(state, playerId) {
  const player = state[playerId];
  if (!player) return { valid: false, reasonKey: 'errors.validation.invalidPlayer' };
  if (player.energyZone.usedThisTurn) return { valid: false, reasonKey: 'errors.validation.energyAlreadyUsed' };
  if (!player.energyZone.currentEnergy) return { valid: false, reasonKey: 'errors.validation.noEnergyAvailable' };
  // First turn rule: player going first can't attach energy on turn 0
  // TODO-Pocket-Verify: Is this per-player or global turn 0?
  if (state.turn === 0 && playerId === 'player1') return { valid: false, reasonKey: 'errors.validation.noEnergyFirstTurn' };
  return { valid: true };
}
...
export function canRetreat(state, playerId, benchIndex) {
  const player = state[playerId];
  if (!player) return { valid: false, reasonKey: 'errors.validation.invalidPlayer' };
  if (!player.active) return { valid: false, reasonKey: 'errors.validation.noActivePokemon' };
  if (player.retreatedThisTurn) return { valid: false, reasonKey: 'errors.validation.alreadyRetreated' };
  if (benchIndex < 0 || benchIndex >= player.bench.length) return { valid: false, reasonKey: 'errors.validation.invalidBenchIndex' };
  if (!player.bench[benchIndex]) return { valid: false, reasonKey: 'errors.validation.benchSlotEmpty' };

  // Status blocks retreat
  const st = player.active.status;
  if (st === STATUS.SLEEP) return { valid: false, reasonKey: 'errors.validation.cannotRetreatAsleep' };
  if (st === STATUS.PARALYSIS) return { valid: false, reasonKey: 'errors.validation.cannotRetreatParalyzed' };
...
export function canEvolve(state, playerId, handIndex, targetLocation, getCardFn) {
  ...
  if (state.turn <= 1) return { valid: false, reasonKey: 'errors.validation.cannotEvolveFirstTurn' };

  const target = targetLocation === 'active'
    ? player.active
    : player.bench[targetLocation];
  if (!target) return { valid: false, reasonKey: 'errors.validation.targetPokemonNotFound' };

  // Can't evolve a Pokemon played this turn
  if (target.turnPlayed === state.turn) return { valid: false, reasonKey: 'errors.validation.cannotEvolvePlayedThisTurn' };

  // Can't evolve a Pokemon that already evolved this turn
  if (target.lastEvolved === state.turn) return { valid: false, reasonKey: 'errors.validation.alreadyEvolvedThisTurn' };
...
export function executeAttachEnergy(state, playerId, target) {
  ...
  if (!pokemon) return state; // safety
