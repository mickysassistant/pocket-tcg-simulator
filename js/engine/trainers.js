...
TRAINER_EFFECTS['Poké Ball'] = {
  ...
  execute(state, playerId, getCardFn) {
    ...
    player.deck.forEach((cardId, idx) => {
      const c = getCardFn(cardId);
      if (c && c.supertype === 'Pokémon' && c.subtype === 'Basic') basics.push(idx);
    });
    if (basics.length === 0) return newState;
...
TRAINER_EFFECTS['Pokémon Communication'] = {
  ...
  execute(state, playerId, getCardFn, opts = {}) {
    ...
    if (handIdx < 0) return newState;
    const deckPokemon = [];
    player.deck.forEach((id, i) => { const c = getCardFn(id); if (c && c.supertype === 'Pokémon') deckPokemon.push(i); });
    if (deckPokemon.length === 0) return newState;
...
TRAINER_EFFECTS['Pokémon Flute'] = {
  ...
  canPlay(state, playerId, getCardFn) {
    ...
    if (opp.bench.filter(p => p !== null).length >= MAX_BENCH) return false;
  ...
  execute(state, playerId, getCardFn) {
    ...
    const basics = [];
    opp.discard.forEach((id, i) => { const c = getCardFn(id); if (c && c.supertype === 'Pokémon' && c.subtype === 'Basic') basics.push(i); });
    if (basics.length === 0) return newState;
    const pick = basics[Math.floor(Math.random() * basics.length)];
...
TRAINER_EFFECTS['Rare Candy'] = {
  ...
  canPlay(state, playerId, getCardFn, opts = {}) {
    ...
    const positions = [];
    if (player.active) positions.push({ pokemon: player.active, idx: 'active' });
    player.bench.forEach((p, i) => { if (p) positions.push({ pokemon: p, idx: i }); });
    for (const pos of positions) {
      const c = getCardFn(pos.pokemon.cardId);
      if (!c || c.subtype !== 'Basic') continue;
      if (pos.pokemon.turnPlayed === state.turn) continue; // Can't evolve same turn
  ...
  execute(state, playerId, getCardFn, opts = {}) {
    ...
    if (pokemonIndex == null || !evolutionCardId) return newState;
    ...
    if (!oldCard || !evoCard || evoCard.subtype !== 'Stage 2') return newState;
...
TRAINER_EFFECTS['Repel'] = {
  ...
  canPlay(state, playerId, getCardFn) {
    ...
    if (!c || c.subtype !== 'Basic') return false;
...
TRAINER_EFFECTS['Squirt Bottle'] = {
  ...
  execute(state, playerId) {
    ...
    if (idx !== -1) opp.active.energy.splice(idx, 1);
...
TRAINER_EFFECTS['Big Malasada'] = {
  ...
  execute(state, playerId, getCardFn) {
    ...
    if (player.active.status) player.active.status = null;
...
TRAINER_EFFECTS['Sabrina'] = {
  ...
  execute(state, playerId, getCardFn, opts = {}) {
    ...
    if (benchIdx < 0 || !opp.bench[benchIdx]) return newState;
    const oldActive = opp.active;
    opp.active = opp.bench[benchIdx];
    opp.bench[benchIdx] = oldActive;
...
TRAINER_EFFECTS['Erika'] = {
  ...
  execute(state, playerId, getCardFn, opts = {}) {
    ...
    if (!c || c.element !== 'Grass') return newState;
...
TRAINER_EFFECTS['Dawn'] = {
  ...
  execute(state, playerId, getCardFn, opts = {}) {
    ...
    if (benchIdx < 0 || !player.bench[benchIdx] || player.bench[benchIdx].energy.length === 0) return newState;
...
TRAINER_EFFECTS['Lillie'] = {
  ...
  execute(state, playerId, getCardFn, opts = {}) {
    ...
    if (!pokemon) return newState;
    const c = getCardFn(pokemon.cardId);
    if (!c || c.subtype !== 'Stage 2') return newState;
...
TRAINER_EFFECTS['Giant Cape'] = {
  ...
  execute(state, playerId, getCardFn, opts = {}) {
    ...
    const player = newState[playerId];
    const pokemon = opts.pokemonIndex === 'active' ? player.active : player.bench[opts.pokemonIndex];
    if (!pokemon || pokemon.tool) return newState;
...
export function canPlayTrainer(state, playerId, cardId, getCardFn, opts = {}) {
  const card = getCardFn(cardId);
  if (!card || card.supertype !== 'Trainer') return false;
