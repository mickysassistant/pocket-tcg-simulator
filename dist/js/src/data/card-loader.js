 ...
 *
 * @returns {Promise<Map<string, Card>>} Map of card ID -> card object
 */
 ...
 * @returns {Card | null} Card object or null if not found
 */
 ...
 *
 * @returns {Card[]} Array of all card objects
 */
 ...
 * @returns {Card[]} Array of cards from the specified set
 */
 ...
 * @param {'small' | 'full'} size - 'small' or 'full' (default: 'full')
...
export function searchCardsByName(query) {
    ...
    return getAllCards().filter((card) => card.name?.toLowerCase().includes(lowerQuery));
}
/**
 * Get cards of a specific type
 *
 * @param {string} type - Energy type (e.g., "Grass", "Fire", "Water", etc.)
 * @returns {Card[]} Array of cards of that type
 */
export function getCardsByType(type) {
    return getAllCards().filter((card) => {
        // Check main type for Pokemon cards
        if ('types' in card && Array.isArray(card.types) && card.types.includes(type)) {
            return true;
        }
