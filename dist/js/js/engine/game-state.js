 ...
 */
import { MAX_BENCH, MAX_HAND, DECK_SIZE, MAX_COPIES_PER_CARD, DEFAULT_COIN_QUEUE_SIZE, STATE_VERSION, ENERGY_TYPES, STATUS, STATUS_CHECKUP_ORDER, STATUS_DAMAGE, KO_POINTS, POINTS_TO_WIN, TURN_LIMIT } from './constants.js';
/**
...
function hasEnoughEnergy(energy, energyCost) {
    ...
    for (const costType of energyCost) {
        ...
        if (costType === 'C') {
            ...
            if (!energyCopy[costType] || energyCopy[costType] <= 0)
                return false;
            energyCopy[costType]--;
        }
    }
