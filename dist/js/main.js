...
function renderPlayer(player, playerId) {
    ...
    if (player.hand.length === 0) {
        ...
        handZone.innerHTML = player.hand.map((cardId, idx) => {
            ...
            return `
                <div class="pokemon-card mini draggable" draggable="true"
    ...
    if (nxtE)
        nxtE.textContent = player.energyZone.nextEnergy || '—';
    // Points
    const pv = document.querySelector(`.points-value[data-bind="${playerId}.points"]`);
    if (pv)
        pv.textContent = player.points;
}
...
function renderAttackPanel(st) {
    ...
    attackerCard.attacks.forEach((attack, index) => {
        ...
        const energyCostHtml = (attack.energyCost || []).map(type => `<span class="energy-cost-icon energy-${type}">${type}</span>`).join('');
        button.innerHTML = `
            <div>
...
function handleAttackClick(attackIndex) {
    if (state.winner) {
        showErrorModal(t('error.gameOver'), t('alerts.gameOverLoadScenario'), {
    ...
    if (state.winner) {
        setTimeout(() => {
            showErrorModal(t('error.gameOver'), t('alerts.gameOverWinnerWins', { winner: state.winner }), {
...
function exportLog() {
    if (!state.log || state.log.length === 0) {
        showErrorModal(t('error.cannotExport'), t('alerts.noLogEntriesToExport'), {
            severity: ErrorSeverity.WARNING,
...
function handleEnergyDrop(zone, targetPlayer) {
    ...
    if (targetPlayer !== playerId) {
        logError(t('alerts.canOnlyAttachToOwnPokemon'));
        return;
    }
    const check = canAttachEnergy(state, playerId);
    if (!check.valid) {
        logError(t(check.reasonKey));
        return;
    }
    let target;
    if (zone.classList.contains('active-zone')) {
        ...
        logError(t('alerts.dropEnergyOnPokemon'));
        return;
    }
...
function handleCardDrop(zone, targetPlayer) {
    ...
    if (zone.classList.contains('active-zone')) {
        ...
        if (!isBasic) {
            logError(t('alerts.onlyBasicToActive'));
            return;
        }
    ...
    if (zone.classList.contains('bench-slot') || zone.classList.contains('bench')) {
        ...
        if (!isBasic) {
            logError(t('alerts.onlyBasicToBench'));
            return;
        }
...
function handleEvolveDrop(playerId, handIndex, targetLocation, evoCard) {
    ...
    if (!target) {
        logError(t('alerts.noPokemonToEvolve'));
        return;
    }
    const targetCard = getCard(target.cardId);
    if (!targetCard) {
        logError(t('alerts.targetCardNotFound'));
        return;
    }
...
function showErrorModal(title, message, options = {}) {
    const { severity = ErrorSeverity.ERROR, suggestion, error, primaryAction, onClose } = options;
...
function loadScenario(json) {
    ...
    try {
        ...
        showErrorModal(t('error.invalidScenario'), t('alerts.invalidScenario', { error: e.message }), {
            severity: ErrorSeverity.ERROR,
...
function setupEventListeners() {
    ...
    document.querySelector('#step-btn')?.addEventListener('click', () => {
        if (state.winner) {
            showErrorModal(t('error.gameOver'), t('alerts.gameOver'), {
                severity: ErrorSeverity.WARNING,
                suggestion: t('error.suggestionGameOver'),
                primaryAction: {
                    text: t('ui.loadScenario'),
...
function renderCoinQueueModal() {
    ...
    container.innerHTML = queue.map((coin, i) => `<span class="coin-item ${coin ? 'heads' : 'tails'}" data-index="${i}">${coin ? '🪙' : '✖️'}</span>`).join('');
...
function renderCoinPreview() {
    ...
    container.innerHTML = nextCoins.map((coin, i) => `<span class="coin ${coin ? 'heads' : 'tails'}">${coin ? '🪙' : '✖️'}</span>`).join('');
}
// ============================================================================
// EDIT MODE (Task 17)
// ============================================================================
...
function renderEditEnergyList(energies) {
    ...
    container.innerHTML = energies.map((e, i) => `<span class="edit-energy-item energy-${e}" data-index="${i}">${e}</span>`).join('');
    // Click to remove
...
function setupEditModalListeners() {
    ...
    document.querySelector('#edit-pokemon-remove')?.addEventListener('click', () => {
        if (!editTarget)
            return;
        if (!confirm(t('alerts.confirmRemovePokemon')))
            return;
        const { playerId, location } = editTarget;
        const pokemon = location === 'active'
            ? state[playerId].active
